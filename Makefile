...
CONTAINERFILE_DIR := $(shell readlink -f ./src)
CONTAINERFILE := $(CONTAINERFILE_DIR)/$(IMAGE_NAME)/Containerfile
GIT_URL=$(shell sed -n -e 's/"//g; s/.*GIT_URL=//p' "$(CONTAINERFILE)")
...
check_args:
	...
	$(call require_file,$(CONTAINERFILE))

ls: ## List available Containerfiles to build
	@ls -1 "$(CONTAINERFILE_DIR)"
...
print_args:
	...
	$(call print_msg, CONTAINERFILE:    $(CONTAINERFILE))
	$(call print_msg, )
	$(call print_msg, )
...
build: check_args print_args ## Build multi-arch image manifest
	...
	@if test -n "$(GIT_URL)"; then \
		mkdir -p "$(CONTAINERFILE_DIR)/$(IMAGE_NAME)/tmp"; \
		if [ -d "$(CONTAINERFILE_DIR)/$(IMAGE_NAME)/tmp/.git" ]; then \
			git -C "$(CONTAINERFILE_DIR)/$(IMAGE_NAME)/tmp" pull; \
		else \
			git clone "$(GIT_URL)" "$(CONTAINERFILE_DIR)/$(IMAGE_NAME)/tmp"; \
		fi \
	fi
...
clean: check_args ## Remove temp files and local manifests
	...
	@rm -rf "$(CONTAINERFILE_DIR)/$(IMAGE_NAME)/tmp"
	@$(CONTAINER_TOOL) manifest rm $(IMAGE_TAGGED) 2>/dev/null ||:
